...
## v4.2变更摘要（Ch1 外评四诊对症：样句门禁下沉 lint + 呼吸感/首章赌注入倾向库，2026-09-20）

> 起因：Ch1 v7 被外部评审＋读者点名四诊（①句式"一个节奏到底"＝一眼AI ②主角像执行程序 ③"算账"词根复读 ④钩子全是空头支票）。v9 重整后按"**能脚本抓的下沉到 lint、脚本抓不到的进 SKILL**"分流：4 条补丁 + 3 条只落档不动框架。

- **style_lint v2.9**：新增两条 L1 顾问项——①`subject_start_repeat`（句首主语占比 >35%，或同主语连续 3 句起句；"他怎么他怎么"式样句门禁）；②`motif_repeat`（按 lint_config 书籍级词根表统计章内频次，单词上限 4；《第三纪元》词根 `["账","算","亏","价"]`）。阈值经 **v8/v9 两版实测校准**：词根合计 21→5（v8 的"账/算"各 6 次触发，v9 零告警）；句首主语 9.5%→22.2%（上限 35%，两版均过——留作"他"字超载防复发门禁）
- **chapter-writer v4.4**：①5.1 补"**呼吸感可操作版**"——长句＝逗号串联 3-5 分句（60-100 字）承载推理/氛围，短句只留重音（一章 5-8 处），长度变化靠段落层级、不靠分号缝合（指纹降参考，见下条），句首主语不连打；②5.3 补"**首章 immediate stakes**"——开篇 ~300 字内给具体/迫切/关己的即时赌注，章末悬念挂在它上面；检验＝删掉章末悬念句后读者仍能说出"他要什么、失什么"（反例：Ch1 v7 "那笔账"式空头支票）
- **detail-reviewer v1.22（必须同步）**：第 1 层新增"句式呼吸"（长短交错＋句首主语连打，对应 lint v2.9）与"怕与欲是否可见"（具体的怕＋秒懂的欲与失；黄金三章缺失→major）
- **复盘落档（框架已有、执行漏了，不改框架）**：①5.1"长短交替"与 5.3"钩子具体可感"早有要求，v7/v8 执行时漏了；②写手定稿前应自行跑 style_fingerprint 校验，原未执行。三条记入 docs/开场方案v2 第五节与 memory，不进规则文本
- **指纹终裁（同日，用户裁决）★**：Ch1 v9（31 个分号缝合长句）指纹仅 1 维超阈 → 真人读者润色意见要求"分号大部分改句号" → v9.1 句号版指纹 5 维超阈。**证据结论：指纹基线口径（切句只认 `。！？`，分号不切句）天然偏好分号缝合式长句，与本书定稿口味方向相反**。裁决：①指纹由提交前置门禁**降为参考指标**（不作拒收依据；AGENTS 第三节门禁条款 + 第九节新增「指纹基线与风格冲突」小节留证）②黄金三章定稿后用**本书自语料重建派生基线**（build + selfcheck）③风格执行以定稿口味为准——句号为主、长短交错靠段落层级排布，不用分号缝合意群。chapter-writer 5.1 / detail-reviewer 第1层已按此修订
- **实测对照（v8→v9）**：句均长 12.77→34.48、长句占比 1.2%→41.3%、逗号/句号 1.60→3.78、指纹超阈维度 5→1（仅虚词分布 1 维轻微超阈）；lint L0 0 / L1 critical 0
- **设计原则**：读者"像AI"的反馈中，**凡是脚本能抓的一律下沉到 lint**（可回归、可用前后版本校准）；SKILL 只保留脚本抓不到的结构性要求，避免倾向库膨胀出"少用某个词"这类不可判读条目

## v4.1变更摘要（番茄官方方法论吸收：4 项升级 + 参考库入库，2026-09-20）

> 起因：用户提供《番茄小说写作方法论·精读版》（74 篇/13 主题 35 万字，番茄作家专区官方课程）。6 路并行深读全文后与现有框架逐条对照——**约八成内容已被覆盖或已有更强机制，真增量 4 条**——按"判据缺失不采、不重复建设"原则吸收。

- **chapter-writer v4.3**：①5.9 E1 补"禁止道德选择题"边界——两难只取利害层，禁止"读者会分裂站队"的伦理两难（依据：官方课程"百万字 20 道选择题→流失超 80%"；与本书反派灰度设计兼容：现场给恨、灰度后置）；②5.4 补黄金三章开篇量化项（插叙≤300字/倒叙≤500字/事件密度/人物与主角篇幅占比）
- **detail-reviewer v1.21（必须同步）**：新增「第11层附：黄金三章开篇量化」专项（仅 Ch1-3，判罚 minor）
- **quality-reviewer v1.10**：新增通用否决项"读者分裂标记"（reader_split_risk）+ 命名毒点清单（五轻毒/五重毒/五过度，跨画像通用）
- **title-reviewer v1.2**：书名硬阈值（≤15字/免生僻字）+ 起名工具（7思路/数字组合/反差句式）+ 简介工具箱（三写法/六段结构/文艺病自检）；「多书名实验」测试时机记入 open_decisions
- **refSkill/ 入库**：番茄官方参考库（精读版 + 5 个合集）纳入版本管理并登记 AGENTS 目录树
- **未采纳项（记录理由）**：修辞量化（与文风包语料校准冲突）、"掀一埋二"（与窗口≤3 上限冲突）、"结构＞字数"（H1 为平台字数硬门禁）；数据回流类属发布后运营项，暂存参考库
- **设计原则**：对照外部方法论的价值在"**差异定位**"——已有更强机制的不重复建设；冲突项先查既有口径来由（如反派灰度来自第三方评审+实测）再决定吸收/改造/不采

## v4.0变更摘要（技法补录：chapter-writer v4.2 + detail-reviewer v1.20 + chief-editor v1.8，2026-09-20）

> 起因：用户提供的一篇泛网文写作经验分析（主题为"AI 写作缺灵魂"）。逐条比对后判定：**约九成内容与既有规则重叠或不可校验，真缺口只有 4 个**——按"只补可判读条目、不搞体系"原则吸收。

- **chapter-writer v4.2**：5.9 补录 4 条（21 条 → 24 条），每条附判据——①**C1 扩展第三尺度「群体尺度」**：用群体行为变化（行商绕道/路人远避/势力闭门）代替"局势紧张"式直述；②**新增 B5 反差法**：以极致平和覆极致杀机（最险处写反常的安稳，反差点不作解释；与 B3 情绪缓冲带区分——B3 喘气，B5 反衬）；③**新增 B6 大场面段落次序**：起势(硬)→环境微动(软)→生态暗变(软)→人心潜藏(隐形)→剧情推进，仅限大场面/爆发点 beat，次序模板非硬约束；④**新增 C6 同一刺激的差异化反应**：同场景多人对同一刺激的反应句须不可互换（全员"脸色大变"=退回）
- **未采纳项（记录理由，防后续重复引入）★**：①原分析的"隐形软逻辑四层递进"层名体系——只有名词无判据、不可校验，照搬会重演刚瘦身掉的规则膨胀；②"AI 十大永久硬伤"二元论——与本书实测（Ch4-7 overall 9.11-9.44）矛盾；③原分析宣传腔不进入规则文本
- **detail-reviewer v1.20（必须同步）**：第11层新增 B5/B6/C6 三项审核、C1 扩为三条款（含群体尺度），15 项 → **18 项**；检查项 ID 仍与写手一一对应
- **chief-editor v1.8**：步骤8「真人读者随口反馈」新增 1 个**不计分必答**问题——「这章读完后，有没有一个'没被说出口'的东西留在你心里？」（有 → 指出哪处；没有 → 直说没有）。此类"言外之意/余味"维度不交给 LLM 自评（已证分数通胀），只向真读者收集；不阻塞入库，跳过/沉默仍走 `[AUTO-APPROVED]`
- **不追溯**：Ch1-7 旧稿不重写（2026-09-20 重启后已整体归档至 archive/第三纪元_重启_20260920/）；新稿从 Ch1 起按 24 条技法 / 第11层 18 项 + 黄金三章开篇量化执行
...
> 起因：handoff 顶层堆了 **100 个文件、645KB**，其中 **43 个是 Ch1-3 的中间版本**（`style_lint_ch1_v2/v3/v5/v6/v6b/v7`、`fp_check_ch1_v2..v6`、`fingerprint_ch1_tctd` 等），全无子目录；根目录亦有已归档项目的遗留文件。按 Ch4-7 的收敛速度推算，300 章将产生 2400+ 文件，治理成本随时间指数上升。

- **制定 handoff 命名规范**（写入 AGENTS.md 第二节）：①**禁止 `_v2`/`_v6b` 式版本后缀**——迭代就地覆盖同名文件，`git` 已是版本控制，不需要文件名再叠一层 ②章号一律裸数字（`_7.json` 而非 `_ch7.json`）；历史遗留的 `style_lint_ch{N}`/`fp_check_ch{N}` 保留 `ch` 前缀（300 章迁移成本高于收益） ③中间审核产物不长期堆顶层（merge 完成后可移入 `handoff/archive/ch{N}/`）。附 10 类产物的规范文件名对照表，标注"必备"与"条件产生"（如 `fix_audit`/`pre_lint` 只在经历 lint 修复轮次时才有）
- **归档 Ch1-3 的 43 个中间版本** → `handoff/archive/golden3_revisions/`（25 个 lint 卡 + 18 个指纹卡）。**保留不删**——它们是黄金三章七轮精修的过程证据，对复盘有价值。handoff 顶层 100 → 56 个文件
- **散落文件归置**：①根目录 `请客_全文.txt` 移入 `archive/请客_放弃_20260804/output/请客_全文_根目录副本.txt`——**注意它不是归档版的冗余副本**（哈希不同，16 行根目录独有 / 44 行归档独有，两者是同一作品的不同版本），故改名保留而非删除；②`选题构思_当前思考_20260802.md` + `选题推进_AB细化_20260802.md` 移入新建 `docs/topic-notes/`（这两份是《第三纪元》的立项推演记录，此前仅存在于根目录无归属）
- **根目录文件登记补全**：AGENTS.md 目录树补齐 `docs/topic-notes/`、3 个分析类 HTML、`作家分享.txt`、`第三方评价.txt` 的用途说明
- **设计原则**：①**同一份数据不能存在于两处而不标注关系**——`请客_全文.txt` 的教训是"文件名相同≠内容相同"，同名文件必须先哈希比对再判定冗余 ②归档而非删除——过程证据在复盘时的价值高于其所占空间 ③规范要写进文档才算规范，否则下一个 Agent 会重新产生 `_v8`

## v3.6变更摘要（规则瘦身：chapter-writer v4.1 + detail-reviewer v1.19，2026-09-20）

> 起因：chapter-writer 的 5.9「实战验证技法」从 v3.5 起逐轮累积到 **28 条平铺**，且每条新增都带一句"与技法X的区别"——**需要辩解自己与既有条目有何不同，本身就说明存在实质重叠**（技法20 与 12 都管代价、技法27 与 5.8b 都管"行动代心理"、技法26 与 5.5 是同一原则的不同粒度）。这违反本项目"硬约束≤10条、规则数量与执行率成反比"的既有立论，只是发生在倾向库上没被察觉。

- **chapter-writer v4.1**：5.9 由 28 条平铺 → **5 组 21 条**，按**写作决策场景**分组（A章际衔接/B场景节奏/C角色反应/D描写信息/E机制结构）。合并 7 处真重复：13+19→A3、17+3→C1、18+21→C2、14+22→C3、12+20→C4、9+16→D1、7+8→D2。**未删除任何可操作内容**（约束/示例/自检方法全保留），只做结构合并；原 11 处"与技法X的区别"辩解全部消除
- **detail-reviewer v1.19（必须同步）**：第11层检查项 ID 改为与写手一一对应（A2/A3/B1-B4/C1-C5/D1/E1/E2/E4），19 项 → **15 项**。**不同步就会脱钩**——detail-reviewer 按旧编号引用"技法23"而写手已无此编号，两个 SKILL 会静默失效（本次即发现该层原有 19 处旧编号引用）
- **编号乱序修正**：「5.8 完整倾向库索引」原排在 5.9 之后 → 移至末尾改名 5.10；原 5.8b 正名为 5.8（父编号已让出）。全节编号恢复升序
- **硬约束数量表述更正**：第二节"硬约束必须 ≤10 条"与实际 H1-H11 共 11 条不符 → 改为"少而稳"，并加"新增硬约束前必须先合并或降级一条旧规则，不得净增"
- **连带引用同步**：`character-designer`（5.8b→5.8）、`memory/outline.json` 的 `cold_action_structure.rule` 字段
- **不追溯**：Ch6/Ch7 的 `detail_review_{6,7}.json` 按旧 19 项产出，属历史记录不重写；session_pointer 框架里程碑已更新为 v4.1/v1.19 并注明 Ch8 起适用新 15 项
- **设计原则**：①**"辩解成本"是重叠的检测器**——一条规则需要解释"我和另一条有何不同"，通常就该合并 ②分组按**使用时机**而非理论类别（写手搭 beat 时不会去翻"感官类"清单）③跨 SKILL 的编号是硬耦合，改编号必须同步所有引用方
...
> 起因：《征诏之界》Ch1-3 实战数据——detail-reviewer 第10层（真人读者视角）三章执行率为零（detail_review 全部无 reader_experience 字段）；distant_recall lint 规则 2 次标记全是同段落 false positive、0 次回应、0 次真实拦截；三章照样过终审 9.05/9.06/9.04——**靠自觉的尾部检查都会衰减到零**。真人阅读反馈 26 处问题中，25 处现有第1-6层本就有对应检查，问题在执行率而非覆盖率。加检查项治不好执行率——第10层就是这么死的。故做减法。

- **style_lint v2.7**：撤回 distant_recall 规则（配置/分级/检测逻辑全删）——其覆盖的回指过远问题仅占真人反馈 3.8%，ROI 为负；检测职责移交真人读者
- **detail-reviewer v1.12**：撤回第10层（不分散——重叠部分第1/6层本已覆盖，分散只会稀释第1层撞上"超10条执行率衰减"铁律）；第1层新增 1 项"展示-解释冗余"检查（26 处反馈中冗余类的共同模式，第10层唯一值得保留的项，8项→9项仍在安全线内）
- **chief-editor v1.6 + 真人读者随口反馈制**：入库前最后一道门（final-reviewer 后/memory-manager 前）。设计原则：**人的工作只是"读"，其他全部是 AI 的工作**——用户读章节后回"过"或随口写问题（无格式无模板），AI 转结构化修改清单、修完只回改动段落；沉默/跳过=AUTO-APPROVED 入库标"真人未读"，事后问题进卷末复盘；同类问题第 2 次出现 → 写入 chapter-writer 自检清单（反馈直达写作端）。替代 v3.0"每5章定期通读+结构化标注"（用户工作量过大且依赖自觉）
- **设计原则**：①减法优先——实战执行率为零的机制直接撤回，不留纸面覆盖 ②不分散——重叠检查已在头部层，加项即稀释 ③读者体验归真人——LLM 假装读者的输入矛盾不可修复，真读者随口反馈成本最低 ④反馈到写作端闭环——同类问题2次进 writer 自检，不经审核层衰减
...
### 真人读者随口反馈 (Reader Spot Check)（v3.3 重构：入库前+随口反馈制）

**背景**：AI 写作 + AI 审核的固有缺陷——LLM 无法真正以"读者视角"体验阅读流畅度。detail-reviewer 第10层（reader_experience）实战三章执行率为零（《征诏之界》Ch1-3 detail_review 全部无该字段），distant_recall lint 规则 false positive 率 100% 且 0 回应——两者已随 v3.3 一并撤回。**读者体验问题只能由真读者发现，不再由 LLM 层承担。**

**机制（v3.3 随口反馈制，替代 v3.0 定期抽检）**：
1. **位置**：final-reviewer 之后、memory-manager 入库之前——入库前最后一道门
2. **人的全部工作**：①读章节（本来就会读，增量≈零）②回一句"过"，或随口写问题（"第3段读着别扭"——无格式、无模板、无评分表）③不想读就跳过/沉默
3. **AI 的全部工作**：呈交时附一句遗留说明（如"对话占比21%偏低，因战斗场景"）；把随口反馈转成结构化修改清单；修完只回改动段落给用户确认（不重读全章）
4. **不阻塞**：auto 模式沉默 → `[AUTO-APPROVED]` 入库，章节标记"真人未读"；事后读到问题 → 进卷末复盘窗口统一润色（沿用既有卷末复盘机制）
5. **反馈闭环**：同类问题第 2 次被用户指出 → 写入 chapter-writer 自检清单（反馈直达写作端，不经审核层衰减）

**v3.0 旧机制存档**：每 5 章定期通读 + `memory/reader_feedback_log.jsonl` 结构化标注——因用户工作量过大且依赖自觉执行，已由 v3.3 随口反馈制替代。
